...
/*		final Button sendButton = new Button("Send");
	...
	public void print(VerticalPanel dialogVPanel, double latitude, double longitude){
		//Date now = new Date(); 
		latitude = 41.926821;  //41.926821, -73.996886
        longitude = -73.996886;
       // double timezone = now.getTimezoneOffset() / 60 * -1;
        ...
        ArrayList<String> prayerTimes = prayers.getPrayerTimes(time[0], time[1], time[2],
                latitude, longitude, time[3]);
        ArrayList<String> prayerNames = prayers.getTimeNames();
        dialogVPanel.add(new HTML("<div>" + time[3] + " - " + time[0] + "/" + time[1] +"/" + time[2]+ " </div>"));
        for (int i = 0; i < prayerTimes.size(); i++) {
        	dialogVPanel.add(new HTML("<div>" + prayerNames.get(i) + " - " + prayerTimes.get(i)  + " </div>"));
        }
	}
